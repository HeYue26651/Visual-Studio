...
void getch()
{
	if (cc == ll)
	{
		...
		while (ch != 10)//ch不等于换行
		{
			if (EOF == fscanf(fin, "%c", &ch))//fin为空  fscanf()遇到空格和换行时结束
			{
				line[ll] = 0;
				break;
			}
...
void getsym()
{
	...
	if (ch >= 'a' && ch <= 'z')
	{
		...
		do {	//检查当前符号是否为保留字
			k = (i + j) / 2;
			if (strcmp(id, word[k]) <= 0)//比较id和word[k]的大小
			{
				j = k - 1;
			}
...
{
	enum symbol addop;
	if (sym == pluse || sym == minuse)             /*开头的正负号，此时当前表达式被看作一个正的或负的项*/
	{
		addop = sym;                    /*保存开头的正负号*/
		getsym();
		if (addop == minuse)
